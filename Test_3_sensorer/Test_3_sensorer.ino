...
void newRound(){
  ...
  int NewNumber = random(0,3);          // Velger et random tall mellom 0 og 2
  
//  Serial.println("newRound 2");         // --> Fjernes etter debugging
  while (NewNumber == SensorNumber){    // Sjekker om det nye tallet er lik det forrige vi valgte
   NewNumber = random(0,3); // Velger evt nytt tall
  }
  SensorNumber = NewNumber;

  Serial.print("LED nr ");
  Serial.print(SensorNumber);
  Serial.println("  skal lyse");
  
  for (int i=0; i<=2; i++){
    if (SensorNumber == i) {
      digitalWrite(LEDArr[i],HIGH);   // Setter på en valgt LED-pære
    }
  }
}
...
void youWin(){
  ...
  lcd.setCursor(8,0);
  lcd.print(points);             // Skriver ut ny poengsum
}
